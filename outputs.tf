...
output "pas_blobstore_gcp_service_account_key" {
  value = "${var.blobstore_service_account_key == "" ? base64decode(element(concat(google_service_account_key.blobstore_service_account_key.*.private_key, list("")), 0)) : var.blobstore_service_account_key}"
  sensitive = true
}
